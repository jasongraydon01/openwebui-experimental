...
class Pipe:
    ...
    class Valves(BaseModel):
        ...
        refine_response: bool = Field(
            default=True,
            description="If True, review and refine the LLM response to ensure conciseness and proper citations.",
        )
        refine_temperature: float = Field(
            default=0.2,
            description="Temperature for the response refinement step (lower for more focused refinement).",
        )
...
- Is a simple greeting or chitchat""",
    ...
    async def _refine_response(self, initial_response: str, query: str, consolidated_context: str) -> str:
        """
        Refine the LLM's initial response to make it more concise, focused, and properly cited.

        :param initial_response: The original response from the LLM
        :param query: The original user query
        :param consolidated_context: The consolidated context from retrieved documents
        :return: The refined response
        """
        # Create a prompt for the refinement process
        refine_prompt = f"""
...
TASK: Refine the initial response to be more:
1. Concise and directly relevant to the query
2. Factually accurate (based ONLY on the source documents)
3. Well-structured with proper formatting
4. Enhanced with properly formatted hyperlinked citations

FORMAT ALL CITATIONS as clickable markdown links: "[Claim text](one_drive_link)"
Example citation: "The sales forecast shows a 15% increase [according to the Q2 Report (Slide 5)](https://link-to-file)"

IMPORTANT GUIDELINES:
- Remove any redundant information
- Ensure EVERY factual claim has a specific citation link
- Make ALL citations clickable using the exact markdown format
- Begin with a direct answer to the query
- Organize information logically with headers if needed
- If source documents contradict each other, note this and cite both sources

DO NOT add any information not found in the source documents.
"""

        # Prepare messages for the refinement request
        messages = [
            {"role": "system", "content": "You are an expert editor specializing in refining RAG responses. Your job is to make responses more concise, accurate, and well-cited."},
        ...
        payload = {
            "model": self.valves.llm_model,
            "messages": messages,
            "temperature": self.valves.refine_temperature,
        ...
        try:
            ...
            if not needs_retrieval:
                ...
                if body.get("stream", False):
                    # For streaming responses, we can't easily apply the refinement step
                    # as we'd need to collect the entire response first, then send it for refinement
                    # So for now, we just pass through the streaming response
                    def stream_response():
                        for line in response.iter_lines():
                            if line:
                                yield line.decode("utf-8")

                    # If refinement is enabled, add a note about this limitation
                    if self.valves.refine_response and __event_emitter__:
                        await __event_emitter__(
                            {
                                "type": "status",
                                "data": {
                                    "description": "Response refinement is not available in streaming mode",
                                    "done": True,
                                },
                            }
                        )
...
---
"""
                        await __event_emitter__({"type": "message", "data": {"content": doc_message}})
                        ...
                        await __event_emitter__(
                            {
                                ...
                                "data": {
                                    ...
                                    "metadata": [
                                        {
                                            "date_accessed": time.strftime("%Y-%m-%d %H:%M:%S"),
...
REMEMBER: Your primary goal is to answer the query with properly cited information that clearly indicates which document and slide each fact comes from."""
                ...
                if body.get("stream", False):
                    ...
                    if "choices" in result and result["choices"]:
                        ...
                        # If refinement is enabled, refine the response
                        if self.valves.refine_response and needs_retrieval:
                            if __event_emitter__:
                                await __event_emitter__(
                                    {
                                        "type": "status",
                                        "data": {
                                            "description": "Refining response for conciseness and proper citations...",
                                            "done": False,
                                        },
                                    }
                                )
                                ...
                                await __event_emitter__(
                                    {
                                        "type": "status",
                                        "data": {
                                            "description": "Response refined and optimized",
                                            "done": True,
                                        },
                                    }
                                )
